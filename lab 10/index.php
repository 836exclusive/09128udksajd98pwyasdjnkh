...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sport_type = isset($_POST["prm11"]) ? $_POST["prm11"] : ""; // Check if prm11 is set, otherwise set $sport_type to an empty string

// Check if $sport_type is not empty before executing the SQL query
if (!empty($sport_type)) {
  ...
  VALUES ('$surname', '$name', '$patronymic', '$competition_date', '$competition_time', '$previous_participation', '$registration_address', '$email', $age, '$preparation_level', '$sport_type', $team_number, '$parental_consent')";
    ...
    echo "Ошибка: Выберите вид спорта.";
}
        ...
        echo "<th>Удалить</th>";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td><a href='delete.php?id={$row['id']}' onclick='return confirm(\"Вы уверены, что хотите удалить эту запись?\")'>Удалить</a></td>";
                echo "</tr>";
            }
